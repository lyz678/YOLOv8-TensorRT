...
import queue
from servo_control.srv import MoveServo, MoveServoRequest
...
class PersonTracker:
    def __init__(self, args):
        ...
        # 初始化服务客户端
        self.move_servo_client = rospy.ServiceProxy('move_servo', MoveServo)
        rospy.wait_for_service('move_servo')
        ...
        time.sleep(0.1)
        # 初始化舵机2和3到中间位置
        self.move_servo(2, self.servo_mid[2], self.angle_speed)
        self.move_servo(3, self.servo_mid[3], self.angle_speed)
    ...
    def move_servo(self, servo_id, target_position, angle_speed):
        ...
        def send_servo_command():
            try:
                ...
                resp = self.move_servo_client(servo_id, target_position, angle_speed)
                if resp.success:
                    with self.servo_lock:
        ...
        with self.servo_lock:
            if self.servo_moving[servo_id]:
                print(f"舵机{servo_id}正在移动，跳过本次命令")
                return

        # 启动线程发送舵机命令
        threading.Thread(target=send_servo_command, daemon=True).start()

    def return_all_servos_to_end(self):
        """将所有舵机移到结束位置"""
        time.sleep(0.1) # 等待上一任务执行完
    ...
    def perform_scan(self):
        ...
        if current_time - self.last_scan_time > self.scan_interval:
            ...
            self.move_servo(1, new_pos, int(1000 * self.scan_interval))
