...
def move_servo(servo_id, target_position, angle_speed):
    ...
    print(f"Sending command - Servo ID: {servo_id}, Target position: {target_position}, Move time: {command_time_ms}ms")
...
def main(args: argparse.Namespace) -> None:
    ...
    try:
        while not rospy.is_shutdown():
            ...
            # OpenCV默认使用BGR，但ROS可能是RGB，需要转换
            bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
            
            # 处理帧
            has_person = tracker.process_frame(bgr)
